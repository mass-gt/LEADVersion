...
def generate_args(method):
    ...
    if method == 'from_file':   
        ...
        if sys.argv[0] == '' :
            ...
            varDict['SKIMTIME']     = varDict['INPUTFOLDER'] + 'skimTijd_REF.mtx'
            varDict['SKIMDISTANCE'] = varDict['INPUTFOLDER'] + 'skimAfstand_REF.mtx'
            ...
            varDict['SKIMTIME'] 		= varDict['INPUTFOLDER'] + sys.argv[4] #'skimTijd_new_REF.mtx' 		
            varDict['SKIMDISTANCE']		= varDict['INPUTFOLDER'] + sys.argv[5] #'skimAfstand_new_REF.mtx'	
            varDict['ZONES']			= varDict['INPUTFOLDER'] + sys.argv[6] #'Zones_v4.shp'				
            varDict['SEGS']				= varDict['INPUTFOLDER'] + sys.argv[7] #'SEGS2020.csv'
            varDict['DISTRIBUTIECENTRA']  = varDict['INPUTFOLDER'] + sys.argv[8] # 'distributieCentra.csv'				
            varDict['ExternalZones']      = varDict['INPUTFOLDER'] + sys.argv[9] #'SupCoordinatesID.csv'			
            varDict['VEHICLE_CAPACITY']   = varDict['INPUTFOLDER'] + sys.argv[10] #'CarryingCapacity.csv'

            varDict['PARAMS_TOD']         = varDict['INPUTFOLDER'] + sys.argv[11] #'Params_TOD.csv'
            varDict['PARAMS_ET_FIRST']    = varDict['INPUTFOLDER'] + sys.argv[12] #'Params_EndTourFirst.csv'
            varDict['PARAMS_ET_LATER']    = varDict['INPUTFOLDER'] + sys.argv[13] #'Params_EndTourLater.csv'
...
def actually_run_module(args):
    ...
    try:
        ...
        dimCombType = pd.read_csv(
            varDict['INPUTFOLDER'] + 'combustion_type.txt',
            sep='\t')
        ...
        shipments = pd.read_csv(
            varDict['INPUTFOLDER'] + f"Shipments_{varDict['LABEL']}.csv")
        ...
        for car in range(nCarriers):
            for tour in range(nTours[car]):
                ...
                for trip in range(nTripsPerTour[car][tour]):
                    ...
                    dwellTime=np.random.gamma(avgDwellTime, 1.2)
                    if dwellTime < 0.08:
                        dwellTime=0.08
...
if __name__ == '__main__':
    # Run the module
    print('Tour formation started')
    root = ''
    main(varDict)
    print('\nTour formation Ended')

# print('Tour Formation ')
# actually_run_module(args)


# print('Tour Formation Completed')
