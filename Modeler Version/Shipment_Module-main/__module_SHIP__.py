...
def generate_args(method):
    ...
    if method == 'from_file':   
        ...
        if sys.argv[0] == '' :
            ...
            varDict['COST_VEHTYPE']        = varDict['INPUTFOLDER'] + 'Cost_VehType_2016.csv'
            varDict['COST_SOURCING']       = varDict['INPUTFOLDER'] + 'Cost_Sourcing_2016.csv'

            varDict['NUTS3_TO_MRDH']       = varDict['INPUTFOLDER'] + 'NUTS32013toMRDH.csv'
            varDict['VEHICLE_CAPACITY']    = varDict['INPUTFOLDER'] + 'CarryingCapacity.csv'
            varDict['LOGISTIC_FLOWTYPES']  = varDict['INPUTFOLDER'] + 'LogFlowtype_Shares.csv'

            varDict['PARAMS_TOD']     = varDict['INPUTFOLDER'] + 'Params_TOD.csv'
            varDict['PARAMS_SSVT']     = varDict['INPUTFOLDER'] + 'Params_ShipSize_VehType.csv'
            varDict['PARAMS_ET_FIRST'] = varDict['INPUTFOLDER'] + 'Params_EndTourFirst.csv'
            varDict['PARAMS_ET_LATER'] = varDict['INPUTFOLDER'] + 'Params_EndTourLater.csv'
            ...
            varDict['ZONES'] = varDict['INPUTFOLDER'] + sys.argv[7] #'Zones_v6.shp'
            ...
            varDict['DISTRIBUTIECENTRA']  = varDict['INPUTFOLDER'] + sys.argv[10] #'distributieCentra.csv'
            varDict['NSTR_TO_LS']         = varDict['INPUTFOLDER'] + sys.argv[11] #'nstrToLogisticSegment.csv'
            varDict['MAKE_DISTRIBUTION']  = varDict['INPUTFOLDER'] + sys.argv[12] #'MakeDistribution.csv'
            varDict['USE_DISTRIBUTION']   = varDict['INPUTFOLDER'] + sys.argv[13] #'UseDistribution.csv'
            varDict['SUP_COORDINATES_ID'] = varDict['INPUTFOLDER'] + sys.argv[14] #'SupCoordinatesID.csv'
            varDict['CORRECTIONS_TONNES'] = varDict['INPUTFOLDER'] + sys.argv[15] #'CorrectionsTonnes2016.csv'

            varDict['CEP_SHARES'] = varDict['INPUTFOLDER'] + sys.argv[16] #'CEPshares.csv'

            varDict['COST_VEHTYPE']        = varDict['INPUTFOLDER'] + sys.argv[17] #'Cost_VehType_2016.csv'
            varDict['COST_SOURCING']       = varDict['INPUTFOLDER'] + sys.argv[18] #'Cost_Sourcing_2016.csv'

            varDict['NUTS3_TO_MRDH']       = varDict['INPUTFOLDER'] + sys.argv[19] #'NUTS32013toMRDH.csv'
            varDict['VEHICLE_CAPACITY']    = varDict['INPUTFOLDER'] + sys.argv[20] #'CarryingCapacity.csv'
            varDict['LOGISTIC_FLOWTYPES']  = varDict['INPUTFOLDER'] + sys.argv[21] #'LogFlowtype_Shares.csv'

            varDict['PARAMS_TOD']     = varDict['INPUTFOLDER'] + sys.argv[22] #'Params_TOD.csv'
            varDict['PARAMS_SSVT']     = varDict['INPUTFOLDER'] + sys.argv[23] #'Params_ShipSize_VehType.csv'
            varDict['PARAMS_ET_FIRST'] = varDict['INPUTFOLDER'] + sys.argv[24] #'Params_EndTourFirst.csv'
            varDict['PARAMS_ET_LATER'] = varDict['INPUTFOLDER'] + sys.argv[25] #'Params_EndTourLater.csv'

            varDict['ZEZ_CONSOLIDATION'] = varDict['INPUTFOLDER'] + sys.argv[26] #'ConsolidationPotential.csv'
...
def actually_run_module(args):
    ...
    try:
        ...
        dimNSTR = pd.read_csv(
            varDict['INPUTFOLDER'] + 'nstr.txt',
            sep='\t')
        dimLS = pd.read_csv(
            varDict['INPUTFOLDER'] + 'logistic_segment.txt',
            sep='\t')
        dimShipSize = pd.read_csv(
            varDict['INPUTFOLDER'] + 'shipment_size.txt',
            sep='\t')
        dimVT = pd.read_csv(
            varDict['INPUTFOLDER'] + 'vehicle_type.txt',
            sep='\t')
        dimFT = pd.read_csv(
            varDict['INPUTFOLDER'] + 'flow_type.txt',
            sep='\t')
        ...
        if varDict['SHIPMENTS_REF'] == "":
            superComMatNSTR = pd.read_csv(
                varDict['INPUTFOLDER'] + 'CommodityMatrixNUTS3.csv',
                sep=',')
